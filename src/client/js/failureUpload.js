(() => {
  ...
  const failureBox = document.querySelector('.failure-upload');
  const canvasRow = failureBox.querySelector('.failure-upload__canvas-row');
  const addCanvasBtn = failureBox.querySelector('.failure-upload__add-canvas');
  const finishFailureBtn = failureBox.querySelector(
    '.failure-upload__finish-btn'
  );

  let canvas = failureBox.querySelector('#failure1');
  const failures = [[]];
  ...
  function makeCanvas(canvas) {
    ...
    const white = 2; // 흰돌은 2
    let count = localStorage.getItem('turn') === '1' ? 0 : 1;
    let recordPae = {};
    ...
    canvas.addEventListener('mouseup', (e) => {
      ...
      if (onTheBoard(e.offsetX, e.offsetY)) {
        ...
        const thisId = canvas.id[canvas.id.length - 1];
  ...
  function addCanvas(event) {
    ...
    canvas.id = `failure${failureCount}`;
  ...
  const finishMakeFailure = async (event) => {
    event.preventDefault();
    const question = {
